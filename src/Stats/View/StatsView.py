...
class StatsView(QMainWindow):
    ...
    def load_user_fields(self):
        ...
        self.zero_seventeen.setText(str(round((self.statsM.get_user_by_bdate(date.today(), date.today()- relativedelta(years=17))/user_count)*100, 2)) + '%')
        self.eighteentfive.setText(str(round((self.statsM.get_user_by_bdate(date.today()-relativedelta(years=18), date.today()-relativedelta(years=25))/user_count)*100, 2)) + '%')
        self.tsixfifty.setText(str(round((self.statsM.get_user_by_bdate(date.today()-relativedelta(years=26), date.today()- relativedelta(years=50))/user_count)*100, 2)) + '%')
